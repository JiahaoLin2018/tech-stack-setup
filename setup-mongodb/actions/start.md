...
启动 MongoDB 服务（支持本地和远程两种部署模式）。

## 命令格式

```
/setup-mongodb start [--env <env>] [--host <ip>] [--user <user>] [--password <pass>|--key <path>]
```
...
> **安全警告**：`--password` 方式传递 SSH 密码会留在 shell history 中，生产环境强烈建议使用 `--key` 方式认证。
...
1. **解析 --env 参数**：
   ...
   - 校验合法值：必须为 `dev|sit|fat|uat|prod`，否则立即报错退出：
     ...
     ```
   - 根据 ENV 确定部署目录 `DEPLOY_DIR=/opt/tech-stack/mongodb-${ENV}` 和容器名 `CONTAINER=tech-mongodb-${ENV}`
...
6. **检查远程 .env**：
   ...
   - 存在 → 检查占位符：
     ...
     ```
   - 校验 exporter 关键变量已设置（避免 init JS 跳过创建 exporter 用户导致 :9216 抓取失败）：
     ```bash
     ssh ... "set -a && source ${DEPLOY_DIR}/.env && set +a && \
       [ -n \"\${MONGO_EXPORTER_PASSWORD}\" ] && [ -n \"\${MONGO_APP_PASSWORD}\" ] && [ -n \"\${MONGO_INITDB_ROOT_PASSWORD}\" ]" || {
       echo "❌ MONGO_INITDB_ROOT_PASSWORD / MONGO_APP_PASSWORD / MONGO_EXPORTER_PASSWORD 必须全部设置"
       exit 1
     }
     ```
...
9. **验证 exporter 可抓取**（exporter 容器需用 `.env` 中的 `MONGO_EXPORTER_PASSWORD` 成功连接 MongoDB；连不上即说明初始化 JS 中的 exporter 用户密码与 `.env` 不一致）：
   ```bash
   ssh ... "curl -sf http://localhost:${MONGODB_EXPORTER_PORT:-9216}/metrics | head -1" \
     && echo "✅ mongodb_exporter 指标端点可达" \
     || echo "❌ mongodb_exporter 抓取失败，检查 MONGO_EXPORTER_PASSWORD 是否与初次 init 时一致"
   ```

10. **展示连接信息**：
   ```
   ✅ MongoDB 8.0（${ENV} 环境）已成功部署至远程服务器！

   连接信息（域名方式，推荐）：
     地址：mongodb-${ENV}.renew.com:${MONGO_PORT:-27017}
     管理员用户：${MONGO_INITDB_ROOT_USERNAME} / <请查看远程 .env>
     应用用户：${MONGO_APP_USER}

   连接命令：
     mongosh "mongodb://${MONGO_INITDB_ROOT_USERNAME}:<password>@mongodb-${ENV}.renew.com:${MONGO_PORT:-27017}/admin"

   注意：请确保 DNS hosts.lan 中已配置 mongodb-${ENV}.renew.com 指向 $HOST
   ```
...
当前实例环境：`${ENV}`，域名：`mongodb-${ENV}.renew.com:27017`

> 每个环境（dev/sit/fat/uat/prod）为完全独立的物理实例，通过 `--env` 参数区分。
> 业务 DB 命名建议：`${ENV}_{业务域}`（如 `dev_order`、`prod_log`）。
...
报告文件路径：`<project_root>/env/mongodb-${ENV}.md`（如 `env/mongodb-dev.md`、`env/mongodb-prod.md`）
