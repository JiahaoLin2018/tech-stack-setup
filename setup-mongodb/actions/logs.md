# Action: logs

查看 MongoDB 容器日志（支持本地和远程两种部署模式）。

## 命令格式

```
/setup-mongodb logs [--env <env>] [--host <ip>] [--user <user>] [--password <pass>|--key <path>]
```

## 步骤

1. **解析 --env 参数**（默认 `dev`，合法值 `dev|sit|fat|uat|prod`，传错报错退出）：
   - `DEPLOY_DIR=/opt/tech-stack/mongodb-${ENV}`
   - `CONTAINER=tech-mongodb-${ENV}`

2. **查看最近 50 行日志**：
   ```bash
   ssh ... "docker logs ${CONTAINER} --tail 50 2>&1"
   ```

3. **如需实时跟踪日志**，提示用户手动执行：
   ```bash
   ssh $USER@$HOST "docker logs ${CONTAINER} -f"
   ```

4. **查看慢操作日志**：
   ```bash
   ssh ... "source ${DEPLOY_DIR}/.env && \
     docker exec ${CONTAINER} mongosh --quiet \
     -u \"\${MONGO_INITDB_ROOT_USERNAME}\" -p \"\${MONGO_INITDB_ROOT_PASSWORD}\" \
     --authenticationDatabase admin \
     \"\${MONGO_APP_DATABASE:-appdb}\" \
     --eval \"db.system.profile.find().sort({ts:-1}).limit(10).forEach(printjson)\" 2>/dev/null \
     || echo 'Profiler 未启用或无慢操作记录'"
   ```
