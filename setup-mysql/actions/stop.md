...
停止 MySQL 服务（支持本地和远程两种部署模式）。

## 命令格式

```
/setup-mysql stop [--env <env>] [--host <ip>] [--user <user>] [--password <pass>|--key <path>]
```
...
3. **确认结果**：
   ```bash
   ssh ... "docker inspect ${CONTAINER} > /dev/null 2>&1 && echo '✅ MySQL 容器已停止（容器保留）'"
   ```
